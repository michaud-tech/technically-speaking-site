...
async function anthropic(system, user, maxTokens = 500) {
  ...
  for(let attempt=0;attempt<(grounded?2:1);attempt++){
    ...
    if(!out){if(grounded&&attempt===0)continue;throw new Error('empty_model_response')}
    if(!grounded)return out;
    const m=out.match(/Evidence:\s*["“]([^"”]+)["”]/i),quote=m&&m[1].trim();
    if(quote&&quote.length>3&&user.includes(quote))return out;
  }
  return 'What worked\nYou completed the practice and made your thinking visible.\n\nTry next\nRe-read your response and choose one phrase you could make more specific for Avery.';
}

const coachRule = `EXACT_LEARNER_QUOTE_REQUIRED. You are a practice coach for the TECH communication model. Give brief developmental feedback, never a rewritten answer. Begin with a line formatted exactly as Evidence: "an exact contiguous quote from the learner input". The quoted words must occur verbatim in the learner input supplied below and must be the words your feedback discusses. Use exactly two plain-text headings: "What worked" and "Try next". Do not use Markdown symbols. Under each heading write no more than two short sentences. Name one highest-impact next move. Explain the issue plainly, then leave the learner to solve it. Do not provide sample wording, scripts, an "e.g.", or a replacement line. Do not invent deadlines, facts, stakeholders, or requirements. Treat TECH tools as flexible choices, not a rigid checklist. Critique only content that appears in the learner submission supplied for this action. Never repeat a stock concern simply because it appears in the brief. Never claim the learner wrote or omitted something unless that is visible in the supplied submission. Use they/them for Avery. Do not mention private or unstated facts.`;
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(action==='start-objection'){
      ...
      const answer=await anthropic(`Act as Avery. Give one realistic objection to this ask in 1-2 sentences. Base it on the evidence, false-warning risk, or the cost of delaying onboarding. State the position naturally. Do not agree to sponsor the concept in the objection. Do not explain the hidden interest, coach, or resolve it.`,`BRIEF:\n${BRIEF}\n\nLEARNER ASK:\n${clean(b.ask)}`,220);return res.status(200).json({answer});
    }
    ...
    if(action==='coach-story'){
      ...
      const feedback=await anthropic(`${coachRule} Evaluate the Pain to Promise story. Check that each link follows credibly, the promise stays within what a four-week pilot can establish, and the story lands on a decision Avery can make. Do not demand the labels appear in the final pitch.`, `TODAY: ${clean(b.today)}\nEVERY DAY: ${clean(b.everyDay)}\nIMAGINE IF: ${clean(b.imagine)}\nBECAUSE OF THAT: ${clean(b.because)}\nSO THAT: ${clean(b.soThat)}\nASK: ${clean(b.ask)}`,500);return res.status(200).json({feedback});
    }
